...
using GenericApp.RadiumRCMAudit.Entities;
using GenericApp.RadiumRCMAudit.Main.Components;

namespace GenericApp.RadiumRCMAudit.Main.Models {
  [XmlRoot("item")]
  ...
  public class AuditExceptionModel {
    public int AuditExceptionId {
      get;
      set;
    }
    public int AuditExceptionTypeId {
      get;
      set;
    }
    ...
    public string AuditExceptionType {
      get;
      set;
    }
    ...
    public string AuditExceptionTypeName {
      get;
      set;
    }
    public string UserNameFromRadium {
      get;
      set;
    }
    public string UserNameFromMaster {
      get;
      set;
    }
    public string FacilitiesFromRadium {
      get;
      set;
    }
    public string FacilitiesFromMaster {
      get;
      set;
    }
    public string TemplatesFromRadium {
      get;
      set;
    }
    public string TemplatesFromMaster {
      get;
      set;
    }
  }
...
#region DB Ops
    ...
    public int UpdateAndSave() {
      var existingAudit = _unitOfWork.Audits.GetAll()
                              .Where(a => a.AuditId == this.AuditId)
                              .ToList()
                              .FirstOrDefault();
    ...
    static public AuditViewModel GetAudit(IUnitOfWork unitOfWork, int auditId) {
      var audit = unitOfWork.Audits.GetAll().Where(a => a.AuditId == auditId).Select(a => new AuditViewModel{AuditId = a.AuditId, AuditStartDate = a.AuditStartDate, AuditEndDate = a.AuditEndDate, InternalUserId = a.AuditUserId, Name = a.Name, ExtractId = (int) a.ExtractId}).SingleOrDefault();
    ...
    static public List<AuditExceptionModel> GetAuditExceptions(IUnitOfWork unitOfWork, int auditId) {
      var audits = unitOfWork.AuditExceptions.GetAll().Where(a => a.AuditId == auditId).Select(e => new AuditExceptionModel{AuditExceptionId = e.AuditExceptionId, AuditExceptionType = e.AuditExceptionType.Description, Message = e.Message, AuditExceptionTypeName = e.AuditExceptionType.Name, AuditExceptionTypeId = e.AuditExceptionTypeId, DisplayOrder = (int) e.AuditExceptionType.DisplayOrder, UserNameFromRadium = e.UserNameFromRadium, UserNameFromMaster = e.UserNameFromMaster, FacilitiesFromRadium = e.FacilitiesFromRadium, FacilitiesFromMaster = e.FacilitiesFromMaster, TemplatesFromRadium = e.TemplatesFromRadium, TemplatesFromMaster = e.TemplatesFromMaster}).OrderBy(e => e.DisplayOrder).ToList();
    ...
    static public List<AuditMasterItemModel> GetAuditMasterItems(IUnitOfWork unitOfWork, int auditId) {
      var items = unitOfWork.AuditMasterItems.GetAll().Where(i => i.AuditId == auditId).Select(i => new AuditMasterItemModel{AuditMasterItemId = i.AuditMasterItemId, Login = i.Login, LastName = i.LastName, FirstName = i.FirstName, Facilities = i.Facilities, Templates = i.Templates}).ToList();
...
#region Business Logic
    ...
    /// <summary>
    /// Builds delimited string of Radium templates for use in comparison.
    /// </summary>
    /// <param name="templates">The Radium templates embedded in other text.</param>
    /// <param name="auditName">Name of the Audit.</param>
    /// <returns>Delimited string of Radium templates for comparison</returns>
    static public string CalculateRadiumTemplatesForComparison(string templates, string auditName) {
      ...
      if (isError) {
        ...
        string msg = string.Format("One or more non-numeric Radium Template was found during audit {0}: {1}", auditName, templates);
    ...
    /// <summary>
    /// Builds delimited string of Radium facilities for use in comparison.
    /// </summary>
    /// <param name="facilities">The Radium facilities embedded with other text.</param>
    /// <param name="auditName">Name of the Audit.</param>
    /// <returns>Delimited string of facilities to use in comparison</returns>
    static private string CalculateRadiumFacilitiesForComparison(string facilities, string auditName) {
      if (string.IsNullOrWhiteSpace(facilities)) {
        return (string.Empty);
      }

      string RadiumFacilitiesDelim = Properties.Settings.Default.RadiumFacilitiesDelim [1]
                                         .ToString();
      List<string> exceptions = new List<string>();
      string facResult = Utility.StringToSortedIntString(facilities, RadiumFacilitiesDelim, ref exceptions).Trim();

      if (exceptions.Count > 0) {
        string msg = string.Format("One or more non-numeric Radium Facility was found during audit {0}: {1}", auditName, facilities);
        ErrorTools.HandleError(new ApplicationException(msg), ErrorLevel.NonFatal);
      }
...
#region Audit
    ...
    public void PerformAudit() {
      ...
      foreach (ExtractItemModel extractItem in extractInclude) {
        string optTemplates = CalculateRadiumTemplatesForComparison(extractItem.SecurityGroup, this.Name);
        string optFac = CalculateRadiumFacilitiesForComparison(extractItem.EmployeeID, this.Name);
        ...
        if (!databaseMaster.ContainsKey(extractItem.UserName.ToLower())) {
          ...
          ex.UserNameFromRadium = extractItem.UserName;
          ex.UserNameFromMaster = string.Empty;
          ex.FacilitiesFromMaster = string.Empty;
          ex.FacilitiesFromRadium = optFac;
          ex.TemplatesFromRadium = optTemplates;
      ...
      foreach (string key in databaseMaster.Keys) {
        if (!databaseMaster[key].ProcessedYN && !extractExclude.ContainsKey(key)) {
          ...
          ex.UserNameFromRadium = string.Empty;
          ex.FacilitiesFromMaster = dbFac;
          ex.FacilitiesFromRadium = string.Empty;
          ex.TemplatesFromRadium = string.Empty;
          ex.TemplatesFromMaster = dbTemplates;
          _unitOfWork.AuditExceptions.Add(ex);
        }
      }
      ...
      foreach (ExtractItemModel extractItem in extractExclude.Values) {
        ...
        if ((extractItem.EmployeeID ?? string.Empty).Trim().ToLower() == "multiple") {
          ...
          ex.FacilitiesFromRadium = extractItem.EmployeeID;
        } else {
          string optFac = CalculateRadiumFacilitiesForComparison(extractItem.EmployeeID, this.Name);
      ...
      //commit everyting - may take a minute
      _unitOfWork.Save();
    }
